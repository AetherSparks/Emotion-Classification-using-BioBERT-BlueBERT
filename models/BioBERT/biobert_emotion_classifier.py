...
class BioBERTTrainer:
    ...
    def save_results(self, metrics, save_dir="results"):
        ...
        metrics_file = os.path.join(save_dir, f"biobert_metrics_{timestamp}.json")
        with open(metrics_file, 'w') as f:
            json.dump(metrics, f, indent=2, default=str)
        print(f"✅ Metrics saved to: {metrics_file}")
        
        # Save detailed report
        report_file = os.path.join(save_dir, f"biobert_report_{timestamp}.txt")
        ...
        # Save model
        model_file = os.path.join(save_dir, f"biobert_model_{timestamp}.pth")
        torch.save({
