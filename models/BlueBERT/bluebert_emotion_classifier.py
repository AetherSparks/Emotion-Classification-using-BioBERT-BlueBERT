...
class BlueBERTTrainer:
    ...
    def initialize_model(self, num_classes):
        """Initialize the BlueBERT model with balanced regularization"""
        print(f"\n🤖 INITIALIZING BLUEBERT MODEL WITH BALANCED REGULARIZATION...")
        print(f"  Model: {self.model_name}")
        print(f"  Classes: {num_classes}")
        print(f"  Dropout rate: 0.4 (balanced for learning)")
        print(f"  🩺 Medical domain pre-training: BlueBERT advantage")
        ...
        print(f"  🔧 Moderate dropout to prevent underfitting")
        
        return self.model
    
    def train_model(self, train_loader, val_loader, epochs=3, learning_rate=1e-5, patience=5):
        """Train the BlueBERT model with balanced stability and performance"""
        print(f"\n🚀 TRAINING BLUEBERT MODEL WITH BALANCED SETTINGS...")
        print(f"  Epochs: {epochs} (with balanced early stopping)")
        print(f"  Learning rate: {learning_rate} (moderate for stability)")
        print(f"  Early stopping patience: {patience} epochs (longer for convergence)")
        print(f"  Balanced regularization: ENABLED")
        print(f"  🩺 BlueBERT specific: Medical domain pre-training")
        ...
        for epoch in range(epochs):
            ...
            # Check for class collapse (BlueBERT specific issue)
            if val_accuracy <= 0.34:  # Close to 1/3 (random guessing)
                print(f"   🚨 INFO: Low accuracy detected (accuracy ≤ 34%)")
                if epoch > 2:  # Not first few epochs
                    print("   🔧 Consider: Model may need more epochs to converge")
    ...
    def save_results(self, metrics, save_dir="results"):
        ...
        metrics_file = os.path.join(save_dir, "bluebert_metrics.json")
        with open(metrics_file, 'w') as f:
            json.dump(metrics, f, indent=2, default=str)
        print(f"✅ Metrics saved to: {metrics_file}")
        
        # Save detailed report
        report_file = os.path.join(save_dir, "bluebert_report.txt")
        ...
        # Save model
        model_file = os.path.join(save_dir, "bluebert_model.pth")
        torch.save({
    ...
    def plot_confusion_matrix(self, cm, save_dir):
        ...
        cm_file = os.path.join(save_dir, "bluebert_confusion_matrix.png")
    ...
    def plot_training_history(self, save_dir, test_accuracy=None):
        ...
        ax1.set_title('BlueBERT - Training & Validation Loss')
        ...
        ax2.set_title('BlueBERT - Validation Accuracy During Training')
        ...
        history_file = os.path.join(save_dir, "bluebert_training_history.png")
